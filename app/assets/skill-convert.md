...
分类路径中的 `{{}}` 页码表达式（如 `{{page}}`、`{{(page-1)*20}}`、`{{page == 1 ? "" : page + ".html"}}` 首页免页码写法）均支持：各路径表达式一致时走 `_type` 共享模板并在 requestInfo 内联翻译后的表达式；不一致时逐行保留原式，requestInfo 运行时按 page/key 求值。选择器型占位符（`@@`/`$` 等）在分类入口无法求值，保留并提示。路径可能含 percent-encoding（先解码还原中文，如 `%e8%bf%9e%e8%bd%bd` → `连载`）。
